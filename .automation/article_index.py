...
Source de vérité : les pages d'articles publiées (blog/<slug>.html), datées
par leur en-tête. Les index de blog (y compris leur pagination) en sont
dérivés par generate_blog_indexes.py.

Utilisée par generate_sitemap.py, generate_feeds.py, generate_blog_indexes.py,
update_article_blocks.py et publish.py. Aucune dépendance hors stdlib.
"""
...
# Pages du dossier blog/ qui ne sont pas des articles.
...
def ordered_slugs(base: Path) -> list[str]:
    """Slugs des articles publiés, du plus récent au plus ancien.

    L'ordre est dérivé de la date affichée par chaque page FR ; à date égale,
    ordre alphabétique inverse des slugs (stable et déterministe).
    """
    entries = []
    for page in (base / 'blog').glob('*.html'):
        if NON_ARTICLE_RE.match(page.name):
            continue
        date_span = DATE_SPAN_RE.search(page.read_text(encoding='utf-8'))
        if not date_span:
            raise ValueError(f'date introuvable dans {page}')
        entries.append((parse_date_label(date_span.group(1)), page.stem))
